...
public class ClienteController {
    ...
    public ResponseEntity<String> loginCliente(@RequestBody Map<String, String> loginData) {
        ...
        try {
            boolean isAuthenticated = clienteDAO.verificarSenha(cpf, senha);
            if (isAuthenticated) {
                return ResponseEntity.ok("Login realizado com sucesso!");
            } else {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Cliente não encontrado ou senha incorreta");
            }
